...
namespace UAsset
{
    ...
    public static class PathManager
    {
        ...
        ///     安装包数据目录
        /// </summary>
        public static string PlayerDataPath { get; set; } = $"{Application.streamingAssetsPath}";
        ...
        public static string GetAssetPath(string path)
        {
            ...
            {
                var bundle = Versions.GetBundle(path);
                if (bundle != null)
                {
                    if (bundle.IsVariant && bundle.variant != Versions.CurrentVariant)
                    {
                        path = path.Replace($"/{bundle.variant}", $"/{Versions.CurrentVariant}");
                    }
                }
            }
