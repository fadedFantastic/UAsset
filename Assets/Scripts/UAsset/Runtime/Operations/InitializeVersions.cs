...
namespace xasset
{
    ...
    public class InitializeVersions : Operation
    {
        ...
        private void LoadServerVersions()
        {
            ...
            var severVersionValid = server.timestamp > 0 && server.timestamp >= _local.timestamp;
            // 服务器的版本比本地新的时候（一样新的时候也执行，更新版本号），清单放到待加载的队列。
            if (_local == null || severVersionValid)
            {
                ...
                Versions.LoadRemoteVersions(server);
            }
